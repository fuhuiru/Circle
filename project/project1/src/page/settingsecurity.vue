<template>
  <div>
    ...
    <form v-if="edit" @submit.prevent="changePassword">
  ...
  methods: {
    ...
    changePassword() {
      ...
      api("user/find", { id: user.id, only: ["password"] }).then(r => {
        ...
        error.old = false;
         if(this.validateNewPassword() || this.validateNew2Password())
      return;
      this.pedding = true;
    ...
    validateNewPassword() {
      return this.errorPassword.new = this.currentPassword.new.length < 6;
    },
    validateNew2Password() {
      return this.errorPassword.new2 = this.currentPassword.new != this.currentPassword.new2;
    }
  }
};
</script>
